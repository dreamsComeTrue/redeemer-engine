...
//------------------------------------------------------------------------------------------------------------------------
#ifndef _R_INPUT_KEYBOARD_H_
#define _R_INPUT_KEYBOARD_H_
...
namespace REDEEMER
{
	namespace INPUT
	{
		...
		*/
		...
		/*!	Keyboard events are used in conjunction with keyboard listeners
		*/
		class C_KeyboardEvent
		{
		public:
			/*!	Constructor
			*/
			C_KeyboardEvent (EKeyCode keyCode, unsigned int character);

			/*!	Destructor
			*/
			virtual ~C_KeyboardEvent();

			//	Public data fields
			
			const EKeyCode	Key;	///<	KeyCode of event												
			unsigned int	Text;	///<	Text character
		};

		/*!	Keyboard listeners are used to retrieve buffer input from device. They should be attached to keyboard device using
		**	AttachKeyboardListener method
		*/
		class C_KeyboardListener
		{
		public:
			virtual ~C_KeyboardListener ();

			/*!	Occurs, when key is pressed
			*/
			virtual bool KeyPressed (const C_KeyboardEvent &event) = 0;

			/*!	Occurs when key is release
			*/
		...
		/*!
		**	\brief Base class for keyboard device
		*/
		class C_KeyboardDevice : public C_InputDevice
		{
		public:
			...
			*/
			...
			/*!	Attaches new keyboard listener
			**	\param	keyboardListener Pointer to a class derived from C_KeyboardListener
			*/
			virtual void AttachKeyboardListener (C_KeyboardListener *keyboardListener);

			/*!	Returns list of attached listeners
			*/
			std::vector<C_KeyboardListener*>& GetKeyboardListeners ();

			/*!	Returns key's string representation - for example, KC_ENTER will be "Enter".
			**	\param key KeyCode to convert
			*/
			virtual const std::string& GetKeyCodeAsString (EKeyCode key) = 0;

			/*!	Checks if given modifier is down
			*/
			bool IsModifierDown (EKeyboardModifier modifier);

		protected:	
			/*!	Private constructor
			*/
			C_KeyboardDevice (const std::string &vendorName, bool isBuffered, C_InputSystem* inputSystem);

			unsigned int						m_Modifiers;	///<	Bit field that holds status of Alt, Ctrl, Shift
			std::vector<C_KeyboardListener*>	m_Listeners;	///<	Attached listeners
		};

	}	//	namespace INPUT
}	//	namespace REDEEMER

#endif	//	_R_INPUT_KEYBOARD_H_
